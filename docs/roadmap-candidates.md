...
## C5 — 토큰 저장 방식 재검토 후보 (논의 중, 2026-07-27)

백엔드 개발자 제안으로 논의 진행 중.
...
미확정 — 시점 재조율 중:
- httpOnly 쿠키 + refresh token 회전으로 전환하는 것 자체는 방향 합의됨
  (백엔드 변경 사항, 백엔드가 구현). 다만 시점을 처음엔 '배포 단계'로
  제안받았으나, 실 백엔드 연결 QA 시점에 미리 구현·검증해두는 게 낫다는
  의견을 전달 중 — 백엔드 확인 대기.
