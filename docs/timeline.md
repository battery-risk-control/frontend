...
## 문서 규율 규칙화 — 보고 근거 제시/전수 grep 갱신/상태 어휘 통일 (2026-07-27)

이번 세션 리뷰를 통해 발견된 반복 패턴(근거 없는 "확인했습니다" 보고, 값 변경 시
일부 문서만 갱신, roadmap-candidates.md의 상태 표기가 항목마다 제각각) 6개를 규칙화해
CLAUDE.md/qa-checklist.md/roadmap-candidates.md에 반영했다.

- **A-1**: 문서-코드 일치/불일치 보고 시 grep 결과·파일:줄번호 근거 제시 의무화
  (CLAUDE.md "개발 시 참고 원칙").
- **A-2**: 값/컴포넌트명/prop 변경 시 관련 `docs/*.md` 전수 grep 갱신 의무화 (同).
- **B-1**: ①②③ 단계 표기가 FE 레포 내부 관리용이며 팀 공식 합의가 아니라는 안내를
  "환경 단계" 섹션 상단에 추가.
- **B-2**: 레포 내 README.md가 루트와 `docs-ref/ui-demo-images/`에 2곳 존재한다는
  안내를 "프로젝트 문서" 목록 근처에 추가(향후 "README" 언급 시 경로 명시 요구).
- **D-1**: `qa-checklist.md`에 "H. 보고/회귀 점검" 항목 신설(지시문 번호별 ✅/❌ 보고,
  레이아웃 값 변경 시 인접 파생값 실측 여부). 이에 맞춰 CLAUDE.md·qa-checklist.md의
  "A~G" 표기를 "A~H"로 함께 갱신.
- **D-2**: `roadmap-candidates.md` 상단에 "상태 어휘(착수 전/조사 중/설계 합의(구현
  대기)/구현 중/해결됨)" 5종을 고정하고, 기존 C1~C13 각 항목 표기를 의미 변경 없이
  통일(예: "미착수"→"착수 전", "해소"/"반영 완료"→"해결됨", "설계 합의 완료, 구현
  대기"→"설계 합의(구현 대기)"). 5종에 안 맞는 C3(방향 3안 미결정)만 특수 상태로
  유지하고 괄호로 사유를 남김.
...
## 2026-08-11 — 로그인 PRISM 3D 광석 비주얼 실험

- 로그인 폼과 인증 동작은 유지하고 좌측 소개 영역만 Three.js WebGL 장면으로 교체했다.
- 저다각 결정 광석이 Y축으로 천천히 회전하며, 내부 발광 코어·궤도·노드가 AI 처리 엔진을 표현한다.
- `VITE_AUTH_PRISM_EXPERIMENT=off` 설정 시 기존 `플랫폼 소개 영역 (준비 중)` 화면으로 즉시 복원된다.
- WebGL 생성 실패 시 CSS 결정 이미지로 자동 대체하며 `prefers-reduced-motion`에서는 회전을 멈춘다.
- 검증: `npm run typecheck`, 로그인 관련 ESLint, `npm run build` 통과. `/auth` 빌드 미리보기에서 WebGL 캔버스 1개와 로그인 폼 공존 및 브라우저 오류 0건 확인.
- `docs/qa-checklist.md` A~H: A 인증 상태 로직 무변경, B 인증 화면 전용 컴포넌트로 한정, C 접근 제어 무변경, D 기존 홈·폼 클릭 요소 유지, E 로그인 스플릿 화면 요구사항 유지, F 카드 UI 아님, G 미사용 상태·import 제거 및 실험 플래그 제공, H 좌우 5:6 레이아웃과 모바일 1열 전환 실측.
- 시각 피드백 반영: 광석을 뾰족한 저다각 결정 형태로 축소하고 외피만 회전하도록 분리했다. 내부 AI 칩은 정면에 고정했으며, 좌측 데이터 신호 3종과 우측 AI 기능 3종을 정적 설명으로 추가했다.
- 중앙 품질 개선: 평면 AI 칩을 제거하고 실제 3D TubeGeometry·노드로 만든 뇌 반구와 회로를 고정 배치했다. 결정 외피에는 RoomEnvironment/PMREM 환경광과 PBR 굴절·감쇠·얇은 iridescence 층을 적용해 내부 오브젝트와 색·반사가 함께 어우러지도록 수정했다.
- 후속 시각 조정: 뇌·회로를 광석 내부 여백에 맞게 축소하고, 자체 3D 튜브 획으로 만든 `PRISM` 워드마크를 하단에 고정 배치했다. 광석 오른쪽에는 설명 레일 직전에서 사라지는 6색 굴절광을 추가하고 좌우 레일 위치를 3.5%로 대칭 정렬했다.
- 최종 피드백 반영: 인공적으로 보이던 6색 광선과 반달형 헤일로를 제거했다. 광석은 높이를 줄이고 폭을 넓혔으며, 내부 뇌를 단일 연속 실루엣으로 재구성·축소했다. `AI`와 각진 직선형 `PRISM`, `PURCHASE RISK INTELLIGENCE & SUPPLY MONITORING` 설명을 고정 배치하고 투명 렌더 패스로 분리해 굴절 중복 없이 선명하게 표시한다.
- 브랜드 문구 위치 정정: 광석 내부 영문 설명을 제거하고 하단 파란 브랜드 영역에 `PURCHASE RISK INTELLIGENCE & SUPPLY MONITORING`과 `AI · BIG DATA · ERP · RAG · RISK INTELLIGENCE`를 추가했다. 내부 뇌 회로는 더 축소하고, `AI`·`PRISM`은 원형 튜브 대신 사각 단면 BoxGeometry 획으로 교체해 굵고 각진 형태로 표시한다.
- 최종 문구 위계 정정: 하단 `PRISM` 위 파란 헤드라인을 `PURCHASE RISK INTELLIGENCE & SUPPLY MONITORING`으로 확장하고, `PRISM` 아래 한국어·영문·기술 키워드 문구는 모두 제거했다. 광석 폭을 소폭 확대하고 뇌 회로를 중앙 정렬한 직선형 회로로 단순화했으며, 작은 `AI`를 뇌 내부에 배치했다.
- 참고 이미지 기반 최종 재구성: 광석 내부 3D 글자와 AI 표기를 시각적으로 제거하고, 고정된 반투명 흰 배너에 굵은 고딕형 `PRISM` 워드마크를 배치했다. 하단에는 대칭형 뇌·회로 SVG와 `AI 리스크 분석 엔진 / 분류 · 평가 · 예측`을 추가했으며, 바깥 브랜드 영역의 한국어 설명 문구를 복원했다. 광석 외피만 회전해 브랜드 레이어의 가독성을 유지한다.
- 내부 배치 보정: 흰 배너·뇌 회로·엔진 설명의 전체 폭을 줄여 회전하는 광석 외곽선 안에 들어오도록 조정했다. 배너의 `PRISM`은 별도 장식 글꼴 대신 엔진 설명과 동일한 기본 고딕 계열을 사용하며 독립된 `AI` 표기는 유지하지 않는다.
- 최종 중앙 오브젝트 보정: 광석 내부의 흰색 배너 박스를 제거하고 진한 남색 `PRISM` 글자만 직접 배치했다. 작은 흰색 뇌·회로 심볼은 글자 아래 중앙에 복원해 광석 면 위에서도 명확한 대비를 유지했다.
- 뇌 회로 및 광석 비율 보정: 잘리던 회로 SVG를 여백이 확보된 완전 대칭 도형으로 다시 그리고 `PRISM` 위로 이동했다. 광석 상단 높이를 낮추고 중단 폭을 넓혀 지나치게 뾰족한 실루엣을 완화했다.
- 크리스탈 재질 고급화: 불투명한 청색과 자체 발광을 낮추고 transmission·IOR·clearcoat·환경 반사를 중심으로 재조정했다. 배경 격자가 은은히 투과되는 옅은 블루 글라스, 절제된 흰색 모서리 반사, 낮은 채도의 내부광으로 플라스틱 느낌을 완화했다.
- 광석형 패싯 재질 보정: 단일 색상의 유리 재질 대신 각 삼각 절단면에 아이스 블루·코발트·딥 네이비 농도를 다르게 부여했다. 투과율을 절제하고 굴절 깊이와 면 경계 반사를 강화해 흐린 유리가 아닌 컷팅 사파이어 광석처럼 보이도록 변경했다.
- 투명 수정 톤 보정: 남색·흰색 절단면의 강한 대비를 제거하고 패싯을 옅은 청회색 범위로 통일했다. 투과율을 높이고 내부층과 모서리 불투명도를 낮춰 배경이 비치면서 굴절과 외곽만 은은히 남는 투명 광석으로 정리했다.
- 로그인 좌측 간소화: 광석 투과율을 한 단계 높이고 표면·내부층·외곽선 불투명도를 낮췄다. 좌우 `DATA SIGNALS` 및 `AI CAPABILITIES` 아이콘·설명 레일을 제거해 중앙 광석과 하단 브랜드에만 시선이 집중되도록 정리했다.
- 광석 내부 AI 라벨 추가: 뇌 회로 위에 흰색 `AI`를 추가하고 아래 `PRISM`과 동일한 각진 고딕 계열과 자간을 적용했다. 크기와 발광은 작게 제한해 `AI → 뇌 회로 → PRISM` 위계가 유지되도록 배치했다.
- PRISM P 로고 포인트 적용: 참고 이미지처럼 `P`의 하단 획에 시안 블루에서 딥 블루로 이어지는 사선 커팅 포인트를 추가했다. 광석 내부 워드마크와 하단 대형 워드마크에 동일하게 적용해 브랜드 형태를 통일했다.
- P 로고 최종 복원: 전체 로고 폰트 변경과 외부 조각형 커팅 시안을 폐기하고 최초의 글자 내부 블루 그라데이션 P로 되돌렸다. 이 스타일은 하단 대형 `PRISM`에만 적용하고 광석 내부 `PRISM`은 단색 남색을 유지한다.
- 하단 브랜드 스펙트럼 보정: 점 3개를 제거하고 한국어 설명 크기를 키웠다. 대형 `PRISM`은 흰색 본체를 유지하면서 글자 내부를 통과하는 원호형 무지개 굴절광을 적용했으며, P 하단의 빨강에서 시작해 M의 보라까지 이어지도록 색 범위·곡률·높이를 조정했다. 광석 내부 워드마크는 단색 남색을 유지한다.
- 3D 프리즘 굴절광 추가: Three.js 장면을 좌우로 확장하고 광석 중심으로 들어오는 이중 흰색 입사광, 반대편으로 분산되는 7색 가산 혼합 광선, 하단의 저농도 반사광을 추가했다. 모든 광선은 중앙 오브젝트 뒤에 배치해 AI·뇌 회로·PRISM 가독성을 유지하며 각 geometry/material을 종료 시 해제한다.
- 굴절광 방향 최종 보정: 좌우 입사광과 7색 방사선을 제거하고 광석 하단 꼭짓점에서 바닥으로 퍼지는 네 개의 저농도 청백색 반사면만 남겼다. 바닥에는 흐릿한 단일 스펙트럼 잔광을 추가했으며, 하단 대형 `PRISM`의 무지개 원호는 제거하고 글자 내부를 사선으로 스치는 흰색·아이스 블루 표면 하이라이트로 교체했다.
- 좌우 단일 광선 재구성: 바닥 반사면과 CSS 스펙트럼 잔광을 제거하고 왼쪽의 넓은 흰색 입사광 한 줄기와 오른쪽의 무지개 굴절광 한 줄기로 단순화했다. 오른쪽 광선은 7개 mesh가 아니라 하나의 세분화된 vertex-color 띠 내부에서 색이 분리되도록 구현했으며 양쪽 끝 폭을 넓혀 존재감을 보강했다.
