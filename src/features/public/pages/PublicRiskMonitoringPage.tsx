...
} from '../../../api/publicRiskMonitoring.api'
...
export function PublicRiskMonitoringPage() {
  ...
  // 한 번에 받아 프론트에서 잘랐는데, 그 방식은 200건이 하드 상한이 되고 첫 로딩도 무겁다.
  // 페이지가 바뀔 때마다 offset으로 다음 구간을 새로 요청한다(구매팀 화면과 동일 패턴).
  useEffect(() => {
    // 필터를 빠르게 바꾸면 이전 요청이 늦게 도착해 최신 결과를 덮어쓸 수 있다.
    let cancelled = false
    async function load(token: string | null, filters: AppliedFilters, pageIndex: number) {
      try {
  ...
  }, [accessToken, confidence, country, days, grade, material, page, reloadToken])

  // 전체 건수 + 국가 필터 후보. 페이지 이동에는 반응하지 않는다 — 건수·후보는 필터에만 달려
  // 있어서, 여기 묶어두면 화살표를 누를 때마다 다시 세는 낭비가 된다(구매팀 화면과 동일).
  useEffect(() => {
    let cancelled = false
    async function loadMeta(token: string | null, filters: AppliedFilters) {
      try {
        const commonFilters = {
          days: filters.days,
          grade: filters.grade === ALL ? undefined : filters.grade,
        ...
        if (cancelled) return
        setTotalCount(total)
        // 데이터가 줄어 현재 페이지가 범위를 벗어났으면 마지막 페이지로 되돌린다.
        ...
        // 보면 후보가 그 페이지의 국가로 쪼그라든다. 필터가 바뀔 때만 나가므로 페이지 이동은 가볍다.
