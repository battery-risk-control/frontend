...
export function LatestNewsPanel({
  ...
  return (
    <ScrollCard
      ...
      // 한 페이지가 최대 5건(모든 소비처가 pageSize=5)이라 높이 상한을 두지 않는다 —
      // 예전 320px 상한은 제목이 여러 줄로 접히는 화면(2/3계층 등)에서 5건 중 3건만 보이고
      // 나머지는 숨은 스크롤에 가려지게 만들었다. 상한을 빼면 페이지의 5건이 모두 보이고,
      // 6건 이상은 애초에 없으므로(그다음은 페이지네이션) 내부 스크롤도 불필요하다.
    >
      {isLoading ? (
