...
/**
 * 비로그인 대시보드 우측 "주요 알림" 목록을 만든다.
 *
 * 1. **멀티에이전트가 끝난 건만 올린다.** 외부신호 점수만 높고 ERP·계약 검증은 아직 안 거친
 *    건을 "심각"으로 띄우면 판정이 끝난 것처럼 보인다 — `multi_agent_completed`가 true이고
 *    종합 등급이 심각·주의인 건만 올린다.
 * 2. **가격 변동성을 `정보`로 함께 올린다.** 뉴스가 조용해도 시세가 흔들리면 알아야 하는데,
 *    그건 뉴스 파이프라인에 안 잡힌다.
 *
...
function toNewsAlerts(events: RiskMonitoringEvent[]): DashboardAlert[] {
  return events
    ...
    .map((event) => ({
      ...
      href: `/public/risk-monitoring?eventId=${event.event_id}`,
    }))
}
 ...
 * 등급으로 나누는 기준은 백엔드에 있고, 화면에서 다시 정하면 같은 숫자를 두 곳이 다르게 판정하게 된다.
 *
  ...
  return summaries
    ...
    .map((summary) => {
      ...
      return {
        ...
        href: '/#material-price-detail-heading',
      }
    })
}
 ...
 *
 * 구현은 `lib/formatCollectedAt`에 있다 — 뉴스 상세 패널도 같은 표기를 써야 한다.
 */
