...
레포에 README.md가 2곳 존재 — 루트(`/README.md`, 기술 스택/실행 방법)와
`docs-ref/ui-demo-images/README.md`(파일 매핑표). 향후 "README" 언급 시 반드시 경로까지
명시한다.
...
이 3단계 분류(①②③)는 FE 레포 내부 관리용 개인 정리이며, 팀 전체의 공식 합의 사항이
아니다. 백엔드 등 외부 소통 시 이 용어(①/②/③단계)를 그대로 쓰지 않고 풀어서 설명한다.
...
- **(생략 불가)** Phase 완료 시 `docs/qa-checklist.md`의 QA 체크리스트(A~H) 전체를 실제로 순회하고, 그 결과를 커밋 메시지 본문에 명시한다. 이 문서를 CLAUDE.md에 `@import`로 참조해두는 것만으로는 실행한 것으로 인정하지 않으며, 매 Phase 종결 보고에 이 점검 결과가 빠지면 안 된다.
...
- 다른 레포(backend)의 작업 내용을 pull(또는 그에 준하는 최초 참조)할 때마다, 코드 변경분뿐 아니라 그 레포의 README.md도 함께 확인한다 — 실행 방법·환경변수·알려진 이슈 등이 코드 diff만으로는 안 드러날 수 있다. (surin은 참고 자료 성격이 달라 이 규칙 대상 아님.)
- 문서와 코드가 "일치/불일치"한다고 보고할 때는 반드시 grep 결과나 파일:줄번호를 함께 제시한다. 근거 없이 "확인했습니다/일치합니다"라고만 쓰지 않는다.
- 값/컴포넌트명/prop이 바뀌면, 그 값·이름을 언급하는 모든 docs/*.md를 grep해 전수 갱신한다. 처음 떠오르는 한 문서만 고치고 끝내지 않는다.