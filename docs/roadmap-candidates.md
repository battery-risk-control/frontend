...
## 상태 어휘 (아래 5개 중 하나만 사용)
착수 전 | 조사 중 | 설계 합의(구현 대기) | 구현 중 | 해결됨
...
## C5 — 토큰 저장 방식 재검토 후보 (설계 합의(구현 대기), 2026-07-27)
...
## C7 — 도트 인디케이터 최하단 섹션 사각지대 (해결됨, 2026-07-27)
...
## C8 — 도트 인디케이터 섹션 전환 구간 통상적 사각지대 2곳 (착수 전, 2026-07-27)
...
## C9 — GlobalRiskBoard 마커 hover 툴팁: 네이티브 Tooltip 채택, useHoverDisclosure/커스텀 오버레이 불필요 (해결됨, 2026-07-27)
...
## C10 — 완전히 겹친 마커 간 hover 전환 미검증 (착수 전, 2026-07-27)
...
## C11 — TanStack Query 미사용 (착수 전 — 패키지만 설치됨, 2026-07-27)

`@tanstack/react-query`는 `package.json`에 의존성으로 설치돼 있고 CLAUDE.md/README.md
"기술 스택" 목록에도 "서버 상태: TanStack Query(`useQuery` 중심)"로 올라 있으나, 실제
코드베이스 전수 확인 결과 `QueryClientProvider`가 앱 어디에도 설정돼 있지 않고 `useQuery`
사용처도 0건이다 — 지금까지 모든 데이터 조회가 동기 mock 함수 호출로만 구현돼 있어
비동기 서버 상태 자체가 아직 코드베이스에 등장하지 않았기 때문(실제 API 연동이 전무했음).

이 조사는 "공개 지도 엔드포인트(`GET /api/v1/public/risk-board`) 연결" 작업의 설계 조사
중 발견됐다 — 이 작업이 최초의 실제 비동기 API 연동이 될 예정이라, `QueryClientProvider`
최초 도입 여부(지금 정석대로 도입할지, 최소 구현으로 미루고 별도 작업으로 분리할지)를
그 작업에서 함께 확정해야 한다. 착수 전.

README.md의 "기술 스택" 목록 문구는 이 조사 결과를 반영해 "패키지 설치됨, 실사용처
