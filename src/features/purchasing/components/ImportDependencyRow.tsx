 ...
 *
 * 기간 탭은 여기서 **선택 상태만** 관리한다 — 공개 대시보드와 달리 이 화면의 가격 데이터는
 * 아직 mock(`purchasing.api.ts`)이라 다시 부를 실 API가 없다. 실 API로 전환할 때 이 상태를
 * 상위 페이지로 올리고 `PERIOD_DAYS[period]`로 재조회를 걸면 공개 대시보드와 같은 동작이 된다.
 *
 * 사용 예:
