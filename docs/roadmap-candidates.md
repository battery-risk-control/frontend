...
## C7 — 도트 인디케이터 최하단 섹션 사각지대 (미착수, 2026-07-27)

PageSectionDots의 rootMargin 보정(36d0fa6) 이후에도 페이지 최하단 섹션 부근(약 936px 구간)에서
어떤 도트도 활성화되지 않는 사각지대가 남아있음. 원인은 관찰 대상(heading)이 짧고 마지막
섹션이라 아래쪽 rootMargin 여유가 부족하기 때문으로 추정 — 별도 조사 필요(예: 마지막 섹션은
rootMargin 규칙 예외 처리, 또는 페이지 하단 도달 시 마지막 도트 강제 활성화).
