...
} from '../../../api/publicMaterialRisk.api'
import type {
  ContractEvidence,
  MaterialRiskDetail,
  MaterialRiskItem,
...
export function PublicMaterialRiskPage() {
  ...
  // 첫 렌더부터 조회가 걸리므로 true로 시작한다(PublicRiskMonitoringPage와 같은 방식).
...
function MaterialDetailView({
  ...
  function handleBriefing() {
    navigate(
      `/public/ai-briefing?source=MATERIAL&ref=${encodeURIComponent(detail.erp_material_id)}`,
    )
  }
      ...
      */}
      ...
      {contract && (
        <section className={styles.detailSection}>
          ...
          {/*
            사람이 읽는 계약명을 앞에 세운다. contract_id는 DB PK라 구매 담당자에게 아무
      ...
      {/*
        "확인이 필요하다"에서 멈추지 않고 무엇을 확인해야 하는지까지 말한다. 근거를 불러온
      ...
      <ul className={styles.clauseList}>
        {evidence.results.map((item, index) => (
          <li key={`${item.document_id}-${item.chunk_index}`} className={styles.clause}>
            ...
            {/*
              유사도 원값을 그대로 보여주지 않는다 — 대신 벡터 검색이 이미 매긴 순서를
              순위로 보여준다. 원값은 title에 남겨 개발 중 확인할 수 있게 한다.
            */}
            <span
...
/**
 * 조항 본문에서 눈에 걸려야 하는 수치. 위약금률·상한·납기일수처럼 판단이 걸리는 숫자만 잡는다.
...
/** null은 "0"이 아니라 "—"로 — 값이 없는 것과 0인 것은 다르다(PublicRiskMonitoringPage와 같은 방침). */
