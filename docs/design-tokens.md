...
각 카드(영역)는 정확히 하나의 스크롤 컨테이너만 갖는다. 그 안의 구조적 하위그룹은 별도 스크롤을 갖지 않고, 개별 요소(리스트 행 등)가 넘칠 때만 "더보기"(`...` 또는 아이콘, 본문을 과도하게 가리지 않는 형태)로 펼친다. 1차 개요에 필요한 정보를 더보기 뒤에 숨기지 않는다(빠른 개요 파악 우선). ScrollCard 공용 컴포넌트가 이 규칙을 구조적으로 보장한다.
