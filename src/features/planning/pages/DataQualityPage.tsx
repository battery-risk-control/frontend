import { fetchDataQualityStatus } from '../../../api/planning.api'
...
export function DataQualityPage() {
  ...
  // ERP 연동/RAG 인덱스/마지막 갱신은 숫자가 아닌 상태 텍스트라 숫자 전용인 KpiSummaryCards에
  // 안 맞아 별도 상태 카드로 표시한다. 자재 커버리지만 진짜 숫자라 KpiSummaryCards를 그대로 쓴다.
  const kpi_summary = [
  ...
  const confidenceItems = status.confidence_distribution.map((item) => ({
    ...
    value: item.ratio,
    value_suffix: '%',
    tone: item.label === '경고' ? ('warning' as const) : item.label === '확정' ? ('neutral' as const) : ('normal' as const),
  }))
    ...
    <div className={styles.page}>
      ...
      <div className={styles.body}>
        ...
        <main id="main-content" className={styles.main}>
          ...
          <RankedBarChart title="신뢰도 라벨 분포" caption="확정/참고/경고" items={confidenceItems} />
          <div className={styles.insightCard}>
            <h2 className={styles.insightTitle}>주의</h2>
            <p className={styles.insightBody}>
              경고 라벨 비중 {warningRatio}% — 출처 교차검증 실패 건은 의사결정 시 참고 정도로만 활용을 권고합니다.
            </p>
          </div>
        </main>
      </div>
